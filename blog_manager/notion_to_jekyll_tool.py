...
def convert_notion_to_jekyll(page_id: str, config_path: str = "config.yaml") -> str:
    """
    Converts a Notion Page to a Jekyll Markdown blog post.
    
    Args:
        page_id: The Notion Page ID or URL to process.
        config_path: Path to the config.yaml file (default is 'config.yaml').
        
    Returns:
        A string message indicating whether the generation was successful and the path where the markdown was saved.
    """
    try:
        print("Initializing components...")
        parser_module = NotionParser()
        image_handler = ImageHandler(config_path=config_path)
        gemini_formatter = GeminiFormatter(config_path=config_path)
        ...
        def write_tag_slug_mapping(taxonomy, eng_title):
            slugs_file = "../wholmesian.github.io/_data/tag_slugs.yml"
            if os.path.exists(slugs_file):
                with open(slugs_file, "r", encoding="utf-8") as rf:
                    content = rf.read()
        ...
        if new_tags:
            ...
            for tag_obj in new_tags:
                ...
                if is_taxonomy_in_slugs(taxonomy):
                    print(f"Skipping tag creation for '{taxonomy}' (already exists).")
                    continue
